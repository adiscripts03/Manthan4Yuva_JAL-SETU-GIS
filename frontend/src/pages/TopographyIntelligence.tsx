  ...
  return (
    <div className="bg-background text-on-surface h-screen w-screen overflow-hidden flex flex-col font-body-sm relative selection:bg-primary-container selection:text-on-primary-container">
      ...
      <main className="flex-1 md:ml-[260px] bg-[#0a0c10] relative z-0 overflow-hidden flex">
        ...
        <div className={`absolute left-margin-page top-margin-page w-[340px] z-30 glass-panel rounded-xl flex-col shadow-2xl overflow-hidden border border-outline-variant/30 bg-surface-container/70 backdrop-blur-md transition-all ${isTopographyMinimized ? 'h-[72px]' : 'max-h-[calc(100vh-48px)] overflow-y-auto'} ${loading ? 'opacity-50' : ''}`}>
          <div className="px-gutter py-panel-padding border-b border-outline-variant/20 bg-surface/50 flex justify-between items-center">
            ...
            <button onClick={() => setIsTopographyMinimized(!isTopographyMinimized)} className="text-on-surface-variant hover:text-primary transition-colors">
              <span className="material-symbols-outlined">{isTopographyMinimized ? 'expand_more' : 'expand_less'}</span>
            </button>
          </div>
        ...
        <div className={`absolute bottom-margin-page left-[380px] right-margin-page z-30 glass-panel rounded-xl shadow-2xl border border-outline-variant/30 bg-surface-container/70 backdrop-blur-md p-panel-padding flex flex-col transition-all overflow-hidden ${isProfileMinimized ? 'h-[52px]' : 'h-[160px]'}`}>
          <div className="flex justify-between items-center mb-2 shrink-0">
            <h3 className="font-label-caps text-[11px] text-on-surface-variant tracking-widest flex items-center gap-2 mb-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setIsProfileMinimized(!isProfileMinimized)} className="text-on-surface-variant hover:text-primary transition-colors">
                <span className="material-symbols-outlined text-[16px]">{isProfileMinimized ? 'expand_more' : 'expand_less'}</span>
              </button>
            </div>
          </div>
